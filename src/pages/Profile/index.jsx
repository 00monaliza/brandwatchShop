...
const Profile = () => {
  ...
  const [notifSettings, setNotifSettings] = useState(() => {
    ...
    } catch { return { push: true, email: false }; }
  });
        ...
        <main className="profile-content">
          ...
          {activeTab === 'orders' && (
            <div className="profile-section">
              ...
              {ordersLoading ? (
                <div className="empty-state"><p>{t('common.loading') || 'Загрузка...'}</p></div>
              ) : orders.length === 0 ? (
                    ...
                    >
                      ...
                      <div className="order-items">
                        {order.items?.slice(0, 3).map((item, idx) => (
                          <span key={idx} className="order-item-name">{item.title || item.name}</span>
                        ))}
              ...
              <div className="settings-list">
                <div className="setting-item">
                  ...
                  <label className="toggle">
                    <input
                      ...
                      onChange={(e) => {
                        const updated = { ...notifSettings, push: e.target.checked };
                ...
                <div className="setting-item">
                  ...
                  <label className="toggle">
                    <input
                      ...
                      onChange={(e) => {
                        const updated = { ...notifSettings, email: e.target.checked };
                        setNotifSettings(updated);
                        localStorage.setItem('notificationSettings', JSON.stringify(updated));
                      }}
                    />
                ...
                <div className="setting-item danger">
                  ...
                  <button
                    ...
                    onClick={async () => {
                      if (!window.confirm(t('profile.deleteConfirm') || 'Вы уверены, что хотите удалить аккаунт? Это действие необратимо.')) return;
                      try {
                        await logout();
                        showToast.success(t('profile.accountDeleted') || 'Аккаунт удалён');
                        navigate('/');
                      } catch {
                        showToast.error(t('profile.deleteError') || 'Ошибка при удалении аккаунта');
                      }
                    }}
                  >
                    {t('profile.delete')}

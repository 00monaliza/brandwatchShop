...
import { getProductImages, getProductImage } from '../../shared/utils/productImage';
...
const ProductDetail = () => {
  ...
  const { addToCart, toggleFavorite: ctxToggleFavorite, isFavorite: ctxIsFavorite } = useCart();
  const { products } = useAdmin();
  const { formatPrice } = useCurrency();
  const [product, setProduct] = useState(null);
  const [selectedImage, setSelectedImage] = useState(0);
  const [isFavorite, setIsFavorite] = useState(false); // synced from context
  ...
  useEffect(() => {
    ...
    if (foundProduct) {
      ...
      setIsFavorite(ctxIsFavorite(foundProduct.id));
      addToRecentlyViewed(foundProduct);
    }
  }, [id, products, addToRecentlyViewed, ctxIsFavorite]);
  ...
  const toggleFavorite = useCallback(() => {
    if (!product) return;
    ctxToggleFavorite(product);
    setIsFavorite(prev => !prev);
  }, [product, ctxToggleFavorite]);
          ...
          <div className="product-info">
            ...
            <div className="product-actions">
              ...
              {product.stock != null && product.stock <= 3 && product.stock > 0 && (
                <p className="stock-warning">{t('product.lowStock', { count: product.stock })}</p>
              )}
